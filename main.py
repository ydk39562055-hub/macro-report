...
def generate_briefing(facts: dict, regime: dict, api_key: str) -> dict:
    ...
    try:
        ...
        prompt = f"""당신은 데이터를 꼼꼼히 읽고 솔직하게 말하는 시장 분석가입니다.

★ 절대 금지
- 한자(株, 債) 사용 금지. 순한글만
- 영어 표현 금지 (Narrowing, Bullish 등)
- "~를 주시하세요", "신중하게 결정하세요", "적절한 타이밍을 기다리세요" 같은 뻔한 말 금지
- "안정적입니다", "좋은 흐름입니다" 같은 근거 없는 낙관 금지
- 데이터에 없는 내용을 지어내지 마세요

★ 반드시 지킬 것
- 모든 문장에 구체적 수치를 넣으세요 (예: "VIX 19.2", "근원CPI 0.20%", "BTC 71,007")
- 데이터끼리 모순되는 점을 솔직하게 말하세요
  예: "지표상으로는 7개 중 7개가 양호하지만, 공포탐욕지수가 16으로 극단적 공포 상태라서 심리와 펀더멘털이 엇갈리고 있어요"
- CPI 수치를 보고 인플레가 진짜 안정적인지 판단하세요. 근원CPI MoM이 0.2% 이상이면 둔화라고 단정하지 마세요
- 구체적인 가격/수치 기준을 제시하세요 (예: "VIX가 22를 넘으면 비중 줄이는 걸 고려해보세요")
- 친근하지만 솔직한 말투 ("~하는 게 좋아요", "~를 눈여겨보세요", "솔직히 ~는 불안한 상황이에요")
...
위 데이터를 전부 읽고, 아래 JSON으로만 응답하세요.
각 항목은 반드시 데이터의 구체적 수치를 인용하고, 그 수치가 왜 중요한지 설명하세요.

{{
  "holder_advice": [
    "이미 투자 중인 사람을 위한 구체적 조언 (수치+이유)",
    "이미 투자 중인 사람을 위한 구체적 조언 (수치+이유)",
    "이미 투자 중인 사람을 위한 구체적 조언 (수치+이유)"
  ],
  "cash_advice": [
    "아직 기다리는 사람을 위한 구체적 조언 (수치+이유)",
    "아직 기다리는 사람을 위한 구체적 조언 (수치+이유)",
    "아직 기다리는 사람을 위한 구체적 조언 (수치+이유)"
  ],
  "watch_points": [
    "이번 주 구체적으로 체크할 수치와 기준선",
    "이번 주 구체적으로 체크할 수치와 기준선",
    "이번 주 구체적으로 체크할 수치와 기준선"
  ]
}}"""
